...
//
//-----------------------------------------------------------
class TUQ_DynamiteFire extends ProjectileFire;

const               mNumGrenades = 1;
...
{
    local TUQ_Dynamite g;
    local vector X, Y, Z;
    local float pawnSpeed;

    g = Weapon.Spawn(class'TUQ_Dynamite', instigator,, Start, Dir);
    if (g != None)
    {
        Weapon.GetViewAxes(X,Y,Z);
...
defaultproperties
{
    ...
    AmmoClass=class'TUQ_DynamiteThrowAmmo'
    AmmoPerFire=1
    FireRate=1

    ProjectileClass=Class'TUQ_Dynamite'
    FireSound=Sound'TUQSounds.Sounds.Tick'
}
