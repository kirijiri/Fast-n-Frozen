...
//
//-----------------------------------------------------------
class TUQ_GUI_LANTab extends UT2K4Browser_ServerlistpageBase;
...
function JoinClicked()
{
    ...
    {
        //!!
        URL = li_Server.PlayerOwner().GetURLProtocol()$"://"$li_Server.Servers[li_Server.CurrentListId()].IP$":"$string(li_Server.Servers[li_Server.SortData[li_Server.Index].SortItem].Port);
        //URL=URL$TUQ_Controller(li_Server.PlayerOwner()).TUQ_URL;

        //log("Hiiiiiiiiiiiiiiiiiiiiieeeeeeeeeeeeeeeeeeeeeeeeeeeeeeer:");
        //log(URL);
        //if( Spectator )
        ...
        li_Server.Controller.CloseAll(false,True);
        li_Server.PlayerOwner().ClientTravel( URL, TRAVEL_Absolute, False );
    }
